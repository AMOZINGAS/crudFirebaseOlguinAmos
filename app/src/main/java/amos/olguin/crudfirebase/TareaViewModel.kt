...
class TareaViewModel: ViewModel() {
    ...
    fun agregarTarea(task: Tarea){
        ...
        viewModelScope.launch(Dispatchers.IO){
            ...
            try{
                ...
                db.collection("tareas").document(task.id).set(task).await()
                _listaTareas.postValue(_listaTareas.value?.plus(task))
    ...
    fun actualizarTarea(task: Tarea){

        viewModelScope.launch(Dispatchers.IO){

            try {

                db.collection("tareas").document(task.id).update(task.toMap()).await()
                _listaTareas.postValue(_listaTareas.value?.map { if( it.id == task.id ) task else it })
